    ...
    for (const folder of folders) {
        if (lstatSync(join(require.main.path, commandsDir, folder)).isDirectory())
            ...
            if (!(command instanceof Command)) {
                ...
                client.logError({ data: `Command file ${join(require.main.path, commandsDir, folder)} is an invalid command. Please make sure all files are setup correctly.`})
                continue;
            }
            client.commands.set(command.name, command);
            if (command.aliases && command.aliases.length > 0) 
                for (const alias of command.aliases) {
                    if (client.aliases.get(alias)) client.logError({ data: `Alias ${alias} has occured more than once. Please make sure you have unique "aliases" properties.`});
