...
module.exports = class Cooldowns {
   ...
   */
  constructor(dbCooldowns, client) {
  ...
  setCooldown(user, command, cooldown, type) {
    if (type === "local") {
      ...
      throw new TypeError(
        'Invalid "type" parameter. Please ensure you pass "global" or "local"',
      );
  }
  ...
  getRemainingCooldown(user, command, type) {
    if (type === "local") {
      ...
      if (this._cooldowns.get(user.id).get(command) === undefined)
        return undefined;
      const remainingTime = parsems(
        this._cooldowns.get(user.id).get(command).valueOf() - Date.now(),
      );
      return remainingTime;
    } else if (type === "global") {
      if (this._globalCoolDowns.get(command) === undefined) return undefined;
      const remainingTime = parsems(
        this._globalCoolDowns.get(command).valueOf() - Date.now(),
      );
      return remainingTime;
    } else
      throw new TypeError(
  ...
  async isOnCooldown(user, command, cooldown, type) {
    if (type === "local") {
      if (
        ...
        this._cooldowns.get(user.id).get(command) !== undefined
      ) {
